...
const SwitchPaymentStatusBtn = ({ ticket }: SwitchPaymentStatusBtnProps) => {
  const { switchTicketPaymentStatus } = useTickets();

  const switchPaymentStatus = (ticket: Ticket) => {
    console.log({ ticket });
  ...
  return (
    <button
      onClick={() => switchPaymentStatus(ticket)}
...
export default SwitchPaymentStatusBtn;
